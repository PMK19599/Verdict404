      ...
      <div className="space-y-2">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-2">
          ...
          <button
            ...
            className={`text-left border rounded px-4 py-3 transition-all ${
          ...
          <button
            ...
            disabled={disabled}
            onClick={() => onSelectTask('agent_action')}
            className={`text-left border rounded px-4 py-3 transition-all ${
              selectedTask === 'agent_action'
                ? 'task-btn-active border-cyan-600 bg-cyan-950/20'
                : 'border-slate-800 bg-slate-900/50 hover:border-slate-700 hover:bg-slate-900/80'
            } ${disabled ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}
          >
            ...
            <p className="text-xs text-slate-500 font-mono">
              Action · Pre-execution policy verification
            </p>
          </button>
      ...
      <div className="space-y-1.5">
        <div className="flex items-center justify-between">
          ...
          <span className="console-label">
            {currentConfig.rules.length} invariants
          </span>
        </div>
          ...
          {currentConfig.rules.map((rule, idx) => (
            <div
              ...
              className={`flex items-baseline gap-3 px-3 py-2 font-mono text-xs ${
                idx < currentConfig.rules.length - 1
        ...
        <div className="flex flex-wrap gap-1.5">
          {currentConfig.presets.map((preset) => {
            ...
            if (preset.expectedVerdict === 'FAIL') {
              ...
              colorClass =
                'border-amber-800/50 text-amber-400 hover:bg-amber-950/40';
              Icon = AlertTriangle;
